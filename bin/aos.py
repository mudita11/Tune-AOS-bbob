        ...
        for i in range(self.n_ops):
            for j in range(len(self.gen_window)-1, len(self.gen_window)-self.max_gen-1, -1):
                ...
                if np.any(self.gen_window[j,:,0] == i):
                    ...
                    if total_success + total_unsuccess != 0:
                        reward[i] += np.sum(self.gen_window[j, np.where(self.gen_window[j,:,0] == i) & np.where(self.gen_window[j, :, self.off_met] != -1) , self.off_met]) / np.array(total_success + total_unsuccess)
        return super().check_reward(reward)
        ...
        for i in range(self.n_ops):
            ...
            for j in range(len(self.gen_window)-1, len(self.gen_window)-self.max_gen-1, -1):
                ...
                if np.any((self.gen_window[j,:,0] == i) & (self.gen_window[j, :, self.off_met] != -1)):
                    ...
                    reward[i] += np.sum(gen_best); # print(reward[i])
            if gen_best != []:
                reward[i] = (1/max_gen) * reward[i]**self.intensity / np.max(gen_best)**self.choice4
        return super().check_reward(reward)
...
def build_probability(choice, n_ops, prob_args):
    if choice == 0:
        return Probability0(n_ops, prob_args["p_min"], prob_args["error_prob"])
    elif choice == 1:
        return Probability1(n_ops, prob_args["p_min"], prob_args["p_max"], prob_args["learning_rate"])
    elif choice == 2:
        return Probability2(n_ops, prob_args["p_min"], prob_args["learning_rate"])
    elif choice == 3:
...
def build_selection(choice, n_ops):
    if choice == 0:
        return Selection0(n_ops)
    elif choice == 1:
        return Selection1(n_ops)
    else:
        raise ValueError("choice {} unknown".format(choice))
